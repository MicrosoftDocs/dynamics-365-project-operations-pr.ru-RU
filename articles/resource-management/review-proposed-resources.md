...
3. В области **Создать резервирование ресурса** установите в поле **Состояние резервирования** значение **Предложено**, а затем выберите **Резервировать**.
    ...
    - В запросе ресурсов рецензент запроса должен изменить статус на **Требует проверки**.
    - На вкладке **Рабочая группа** проекта значение в столбце **Состояние запроса** универсального участника группы автоматически изменяется на **Требует проверки**.

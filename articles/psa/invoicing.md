...
description: В этой статье представлена информация о выставлении счетов.
author: rumant
ms.custom:
...
> [!NOTE]
> Пакетное выставление счетов в Project Service Automation выполняется только для строк контракта по проекту, которые настроены с помощью расписаний счетов. В строке контракта с методом выставления счетов с фиксированной ценой должны быть настроены вехи. Для строки контракта по проекту с методом выставления счетов с учетом времени и материала потребуется настроить расписание счетов на основе даты. Информация о настройке периодичности выставления счетов в контексте проекта, основанного на строке предложения с расценками, представлена в статье [Предложение с расценками и строки предложения с расценками](basic-quote-lines.md#invoice-schedule). То же самое справедливо для строки контракта на основе проекта.      
